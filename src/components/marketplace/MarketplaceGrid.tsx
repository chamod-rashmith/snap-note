  ...
  if (notes.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-slate-500">Be the first to publish a note on this topic!</p>
      </div>
    );
  }
       ...
       {notes.map((note, index) => (
         ...
         >
           <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] transition-all duration-300 h-full flex flex-col hover:-translate-y-1 relative group">
             {/* Gradient Accent */}
             <div className="h-2 w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 opacity-0 group-hover:opacity-100 transition-opacity" />
             ...
             <div className="p-8 flex-1">
               ...
               <h3 className="text-xl font-bold text-slate-900 mb-3 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-indigo-600 group-hover:to-purple-600 transition-all">
                 {note.topic}
               </h3>

               <p className="text-sm text-slate-500 line-clamp-3 mb-6 leading-relaxed">
                 {note.content.metadata.objective || "No description available."}
               </p>

               <div className="flex items-center gap-4 text-xs font-medium text-slate-400 border-t border-slate-50 pt-4">
                 <div className="flex items-center gap-1">
                    <User size={14} />
                    <span>User {note.ownerId.substring(0,4)}</span>
                 </div>
                 <div className="flex items-center gap-1">
                    <Calendar size={14} />
                    <span>{new Date(typeof note.createdAt === 'number' ? note.createdAt : (note.createdAt as any).seconds * 1000 || Date.now()).toLocaleDateString()}</span>
                 </div>
               </div>
             </div>
